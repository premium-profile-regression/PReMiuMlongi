    ...
    for(unsigned int i=0; i<dataset.nSubjects(); i++){
      ...
      if((params.z(i) == c &&  ii == dataset.nSubjects()) || ( i == ii ) ){
        ...
        if(!cond_RE){
          ...
          //
          //   for(unsigned int b=0;b<nFixedEffects[m];b++)
          //     std::cout << " m "<<m << " b "<<b<< "beta "<<params.beta(m,b, 0, nCategoriesY)<<endl;
          //
          //   for(unsigned int b=0;b<nFixedEffects_mix[m];b++)
          //     std::cout << " m "<<m << " b "<<b << " c "<< c << "betamix "<<params.beta_mix(m, c,b,0,nCategoriesY)<<endl;
          //
          //
          //   std::cout << ii<< " m "<<m<<" yi"<<yi.transpose();
          //   std::cout << ii<< " m "<<m<<" Vi"<<Vi;
          //   std::cout << ii<< " m "<<m<<" prod"<< -0.5*yi.transpose()*Vi.inverse()*yi << " ni_m "<< ni_m << " sigmaE "<<params.SigmaE(m)<<endl;
          // }
          //std::cout << ii<< " m "<<m<<"  dmvnorm "<<dmvnorm<<endl;
